...
struct Arcade<TypeElem> {
    grid: HashMap<Position, TypeElem>,
    computer: Intcode,
    score: TapeElem,
}
...
impl<T> Arcade<T>
where
    T: Default + From<TapeElem> + fmt::Display + Copy,
{
    fn new(filename: &str) -> Arcade<T> {
        Arcade {
            grid: HashMap::new(),
    ...
    fn execute<F>(&mut self, f: &F, show_steps: bool)
    where
        F: Fn(&HashMap<Position, T>) -> TapeElem,
    {
        loop {
            self.computer.execute();
            while self.computer.output_avail() {
                let x = self
                    .computer
                    .get_output()
                    .expect("Computer did not produce output!");
                let y = self
                    .computer
                    .get_output()
                    .expect("Computer did not produce output!");
            ...
            if self.computer.finished {
                ...
                self.computer.supply_input(f(&self.grid));
            }
        }
    }
    ...
    fn insert_quarters(&mut self, quarters: TapeElem) {
        self.computer.tape[0] = quarters;
    }
}
...
fn main() {
    {
        let mut arcade: Arcade<Tile> = Arcade::new("input.txt");
        arcade.execute(&|x| 0, false);
        let num_panels = arcade.grid.values().filter(|e| **e == Tile::Block).count();
        arcade.print();
        println!();
        println!("Number of block tiles: {}", num_panels);
    }
    {
        let mut arcade: Arcade<Tile> = Arcade::new("input.txt");
        arcade.insert_quarters(2);
        arcade.execute(&|g: &HashMap<Position, Tile>| -> TapeElem {
            let mut ball_x = -1;
            let mut paddle_x = -1;
            for (pos, tile) in g.iter() {
                match tile {
                    Tile::Ball => ball_x = pos.x,
                    Tile::Paddle => paddle_x = pos.x,
                    _ => {}
                }
            }
            assert_ne!(ball_x, -1, "Ball position not in grid!");
            assert_ne!(paddle_x, -1, "Paddle position not in grid!");
            if ball_x < paddle_x {
                -1
